...
4. **SKU Único (Catalogo)**: No se puede registrar un producto si su SKU ya existe en la base de datos.
5. **Sincronización de Stock**: Al buscar o guardar un producto, es obligatorio consultar el stock real en inventario usando el cliente Feign.
6. **Filtro de Disponibilidad en catalogo**: Los productos marcados como no disponibles deben ser excluidos de las consultas públicas.
...
| 4. SKU Único | ✅ Cubierta | Pruebas de insensibilidad a mayúsculas|
...
- El cliente Feign (`InventarioClient`) actualmente solo se prueba en escenarios exitosos (Mocks). Queda pendiente probar cómo reacciona el sistema si el microservicio de Inventario se cae o da un error 500.
- **Acción Futura:** 
